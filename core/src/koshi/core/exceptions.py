"""
Exception hierarchy for Koshi Flux
...
class FluxModelError(KoshiException):
...
class KoshiConfigError(KoshiException):
...
class ValidationError(KoshiException):
...
class ParameterError(KoshiException):
...
class MotionProcessingError(KoshiException):
...
class TensorProcessingError(KoshiException):
...
class ResourceError(KoshiException):
...
class KoshiTimeoutError(KoshiException):
...
EXCEPTION_MAPPING = {
    ...
    "config": KoshiConfigError,
    "validation": ValidationError,
    "parameter": ParameterError,
    "motion": MotionProcessingError,
    "tensor": TensorProcessingError,
    "resource": ResourceError,
    "timeout": KoshiTimeoutError,
    "api": APIError
}


def create_exception(error_type: str, message: str, **kwargs) -> KoshiException:
    """
    ...
    """
    exception_class = EXCEPTION_MAPPING.get(error_type, KoshiException)
    return exception_class(message, **kwargs)


def handle_exception(func):
    """
    Decorator to handle exceptions and convert them to appropriate Koshi exceptions.
    ...
    def wrapper(*args, **kwargs):
        try:
        ...
        except KoshiException:
            # Re-raise Koshi exceptions as-is
            raise
        except FileNotFoundError as e:
            raise KoshiConfigError(f"File not found: {e}", file_path=str(e))
        except ValueError as e:
            raise ValidationError(f"Invalid value: {e}")
        except RuntimeError as e:
            if "CUDA" in str(e) or "GPU" in str(e):
                raise FluxModelError(f"GPU/CUDA error: {e}")
            raise KoshiException(f"Runtime error: {e}")
            ...
            if "module" in str(e) and "LogRecord" in str(type(e)):
                raise KoshiException(f"Logging configuration error: {e}", details={"original_exception": type(e)})
            ...
            raise KoshiException(f"Unexpected error: \"{e}\"", details=error_details)
    
    return wrapper 

class SecurityError(KoshiException):
