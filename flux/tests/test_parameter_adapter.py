...
try:
    from flux_motion.bridge.parameter_adapter import (
        FluxParameterAdapter,
        MotionFrame,
    )
    from setups.core.exceptions import ParameterError
    HAS_PARAMETER_ADAPTER = True
except ImportError:
    try:
        # Try alternative import path
        from flux_motion.adapters import FluxParameterAdapter, MotionFrame
        from flux_motion.core import ParameterError
        HAS_PARAMETER_ADAPTER = True
    except ImportError:
...
pytestmark = pytest.mark.skipif(
    not HAS_PARAMETER_ADAPTER,
    reason="Parameter adapter module not available"
)


@pytest.mark.skipif(not HAS_PARAMETER_ADAPTER, reason="Parameter adapter not available")
