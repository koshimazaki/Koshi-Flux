"""Klein with strength ramp - prompt gradually takes over"""
...
OUTPUT_NAME = "ramp_strength"
MAX_FRAMES = 60

# Strength ramp: start preserving video, end with full prompt
STRENGTH_START = 0.3   # Low = preserve input more
STRENGTH_END = 0.9     # High = regenerate more from prompt
...
for i, inp in enumerate(tqdm(frames, desc=OUTPUT_NAME)):
    # Ramp strength from start to end
    t = i / max(len(frames) - 1, 1)
    strength = STRENGTH_START + t * (STRENGTH_END - STRENGTH_START)
    ...
    if i % 20 == 0:
        print(f"Frame {i}: strength={strength:.2f}")
