"""Klein V2V with strength control - diffusers version

Mimics the v2v_full approach but using diffusers pipeline.
Higher strength = more prompt influence, less input preserved.
"""
...
OUTPUT_NAME = "v2v_strength"
MAX_FRAMES = 60

# Key setting - higher = more prompt, less video
STRENGTH = 0.8  # Try 0.7-0.9
...
print("Loading Klein Inpaint pipeline (has strength)...")
...
cap.release()
print(f"Video: {len(frames)} frames, strength={STRENGTH}")

gen = torch.Generator(device="cuda").manual_seed(SEED)
out_frames, ema_frame, anchor = [], None, None

# Create mask (full white = inpaint everything)
...
for i, inp in enumerate(tqdm(frames, desc=OUTPUT_NAME)):
    ...
    result = pipe(
        prompt=PROMPT,
        image=Image.fromarray(init_frame.astype(np.uint8)),
        mask_image=mask,
        strength=STRENGTH,
...
with open(out_path.replace(".mp4", ".json"), "w") as f:
    json.dump({"timestamp": datetime.now().isoformat(), "preset": "v2v_strength", "strength": STRENGTH, "input_video": INPUT_VIDEO, "prompt": PROMPT, "ema_alpha": EMA_ALPHA, "warp_blend": WARP_BLEND, "post_lab": POST_LAB, "frames": len(frames), "fps": fps}, f, indent=2)
