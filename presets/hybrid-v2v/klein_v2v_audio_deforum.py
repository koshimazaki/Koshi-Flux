#!/usr/bin/env python3
"""Klein V2V Audio Deforum - audio-driven Deforum-style motion in latent space.

THE consolidation preset: merges the two proven stacks that never met:
- klein_v2v_deforum's loop: optical flow warp of prev_gen + motion engine
  schedules applied in the 128-channel latent space + anchor color matching
- klein_v2v_audio's bridge: audio/dashboard-JSON/feature-video -> per-frame
  motion schedules (kick->zoom, high->angle, mid->translation, amp->strength)

Motion is applied ONLY in latent space (no pixel pre-zoom), so there is no
zoom double-counting by construction.
...
Usage:
    ...
    python klein_v2v_audio_deforum.py -i video.mp4 --feature-video motion.mp4 -p "prompt"

    # With LoRA + secondary prompt on kicks
...
parser = argparse.ArgumentParser(description="Audio-driven Deforum motion in latent space")
...
parser.add_argument("--output", "-o", default="outputs/v2v_audio_deforum.mp4")
...
# Audio -> motion mapping (same knobs/defaults as klein_v2v_audio)
...
# Deforum-loop knobs (same semantics as klein_v2v_deforum / klein_hybrid_deforum)
parser.add_argument(
    "--video-blend", type=float, default=0.0,
    help="Blend current input frame back into the flow-warped feedback "
         "(0.0 = pure Deforum feedback like v2v_deforum, 0.3-0.5 = follow video more)",
)
parser.add_argument(
...
parser.add_argument(
    "--lora-backend", choices=["auto", "diffusers", "native"], default="auto",
    help="How to bind the LoRA (see klein_utils.apply_lora)",
)
...
with GenerationContext(args.output) as gen:
    ...
    # Record whether the LoRA actually bound (AGENTS.md accuracy guardrail).
    ...
    for i, frame in enumerate(tqdm(frames, desc="AudioDeforum")):
